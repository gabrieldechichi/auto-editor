...
def main():
    ...
    if(len(sys.argv) > 1 and sys.argv[1] in subcommands):
        if(sys.argv[1] == 'create'):
            ...
            args = ParseOptions(
                sys.argv[2:], Log(), 'create', create_options())
        ...
        if(sys.argv[1] == 'levels'):
            ...
            from usefulFunctions import FFmpeg, FFprobe
            args = ParseOptions(
                sys.argv[2:], Log(), 'levels', levels_options())

            TEMP = tempfile.mkdtemp()
            log = Log(temp=TEMP)
            ffmpeg = FFmpeg(dirPath, args.my_ffmpeg, False, log)
            ffprobe = FFprobe(dirPath, args.my_ffmpeg, False, log)
            levels(args.input, args.track, args.output_file,
                   ffmpeg, ffprobe, TEMP, log)
        sys.exit()
    else:
        option_data = main_options()
        args = ParseOptions(sys.argv[1:], Log(
            True), 'auto-editor', option_data)
    ...
    for i, INPUT_FILE in enumerate(inputList):
        ...
        if(args.sample_rate is None):
            ...
            if(sampleRate == 'N/A'):
                sampleRate = '48000'
                log.warning(
                    f"Samplerate wasn't detected, so it will be set to {sampleRate}.")
        else:
        ...
        if(audioFile):
            if(args.force_fps_to is None):
                # Audio files don't have frames, so give fps a dummy value.
                fps = 30
            else:
            ...
            if(fps < 1):
                log.error(
                    f"{INPUT_FILE}: Frame rate cannot be below 1. fps: {fps}")
        ...
        if(chunks is None):
            ...
            hasLoud = combineArrs(audioList, motionList,
                                  args.edit_based_on, log)
            del audioList, motionList

            effects = []
            if(args.zoom != []):
                from cutting import applyZooms
                effects += applyZooms(args.zoom, audioData,
                                      sampleRate, fps, log)
            if(args.rectangle != []):
                from cutting import applyRects
                effects += applyRects(args.rectangle,
                                      audioData, sampleRate, fps, log)
        ...
        if(fps is None and not audioFile):
            ...
            ffmpeg.run(['-i', INPUT_FILE, '-filter:v',
                        'fps=fps=30', constantLoc])
            ...
            if(args.export_as_clip_sequence):
                ...
                for item in chunks:
                    if(speeds[item[2]] == 99999):
                        continue
                    makeAudioFile(
                        INPUT_FILE, [item], appendFileName(newOutput, f'-{i}'))
                    i += 1
            else:
        ...
        def makeVideoFile(input_, chunks, output):
            ...
            if(continueVid):
                ...
                if(args.render == 'av'):
                    if(args.zoom != []):
                        log.error(
                            'Zoom effect is not supported on the av render method.')

                    if(args.rectangle != []):
                        log.error(
                            'Rectangle effect is not supported on the av render method.')
